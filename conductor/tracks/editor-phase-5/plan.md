...
**Edge/vertex → CommandBar:** Requires resolving the boundaryId/vertexId back to a human-readable HexPath notation. Add a helper `boundaryIdToHexPath(boundaryId, model)` and `vertexIdToHexPath(vertexId, model)` in `selection.ts`.
