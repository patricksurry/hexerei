...
export function offsetToCube(
  col: number,
  row: number,
  orientation: Orientation = 'flat-down'
): Cube {
...
export function createRectangularGrid(
  cols: number,
  rows: number,
  orientation: Orientation = 'flat-down',
  firstCol: number = 0,
...
export function parseBoundaryId(id: string): { hexA: Cube; hexB: Cube | null; direction?: number } {
  ...
  if (parts.length === 2 && parts[1].startsWith('VOID/')) {
    return {
      hexA: hexFromId(parts[0]),
      hexB: null,
      direction: parseInt(parts[1].split('/')[1], 10),
    };
  }
